...
func (t *Tournament) SelfEnroll(player *Player) error {
	if t.State != TournamentStateCreated {
		return errors.New("tournament should be created to enroll the player")
	}
	if slices.ContainsFunc(t.Players, func(other *EnrolledPlayer) bool {
...
func (t *Tournament) Start(host *Host) error {
	...
	if t.State != TournamentStateCreated {
		return errors.New("tournament should be created to be started")
	}
	for _, p := range t.Players {
		if !p.HasCommanders {
			return errors.New("all players in tournament should be have commanders submitted to start the tournament")
		}
	}
...
func (t *Tournament) Finish(host *Host) error {
	...
	if t.State != TournamentStateStarted {
		return errors.New("tournament should be started to be finished")
	}
